...
pub unsafe fn sched_init() {
    ...
    SCHED.tasks[shell_id as usize].mode = TASK_MODE_KERNEL;
    let cur_rsp: u64;
...
pub unsafe fn sched_create_task(name: &str, entry: fn(), priority: u8) -> i32 {
    ...
    SCHED.tasks[id].mode = TASK_MODE_KERNEL;
...
pub unsafe fn sched_create_user_task(
    name: &str,
    entry: fn(),
    priority: u8,
    user_stack: *mut u8,
    ...
    SCHED.tasks[id].mode = TASK_MODE_USER;
    SCHED.tasks[id].user_rsp = if user_stack.is_null() {
        0
    } else {
        (user_stack as u64) + user_stack_size as u64
    };

    let user_rsp = SCHED.tasks[id].user_rsp;
    ...
    let kstack_top = SCHED.tasks[id].stack.as_mut_ptr().add(8192) as *mut u64;
    SCHED.tasks[id].rsp0 = kstack_top as u64;
