...
fn panic(info: &core::panic::PanicInfo) -> ! {
    unsafe {
        static mut FILE_BUF: [u8; 256] = [0u8; 256];
        let msg: *const u8 = b"kernel panic\0" as *const u8;
        let file: *const u8;
        let line: i32;
        if let Some(loc) = info.location() {
            let f = loc.file().as_bytes();
            let len = core::cmp::min(f.len(), 254);
            FILE_BUF[..len].copy_from_slice(&f[..len]);
            FILE_BUF[len] = 0;
            file = FILE_BUF.as_ptr();
            line = loc.line() as i32;
        } else {
            file = b"unknown\0" as *const u8;
            line = 0;
        }
        lumie_panic(msg, file, line);
    }
    loop {}
}
...
fn lumie_set_fg(c: u32) {
    unsafe { console::term_set_fg(c); }
}

#[allow(dead_code)]
fn lumie_set_bg(c: u32) {
    unsafe { console::term_set_bg(c); }
}
...
fn lumie_write(s: *const u8) {
    unsafe { console::term_write(s); }
}

#[allow(dead_code)]
fn lumie_writeln(s: *const u8) {
    unsafe { console::term_writeln(s); }
}
...
fn lumie_fs_init() -> i32 {
    unsafe { fs::detect_and_init() }
}

#[allow(dead_code)]
fn lumie_fs_read(path: *const u8, buffer: *mut c_void, max_size: u32) -> i32 {
    unsafe { fs::read_file(path, buffer as *mut u8, max_size) }
}

#[allow(dead_code)]
fn lumie_fs_write(path: *const u8, data: *const c_void, size: u32) -> i32 {
    unsafe { fs::write_file(path, data as *const u8, size) }
}

#[allow(dead_code)]
fn lumie_fs_list(path: *const u8, entries: *mut c_void, max_entries: i32) -> i32 {
    unsafe { fs::list_dir(path, entries as *mut crate::fs::types::LumieDirEnt, max_entries) }
}

#[allow(dead_code)]
fn lumie_fs_exists(path: *const u8) -> i32 {
    if unsafe { fs::exists(path) } { 1 } else { 0 }
}

#[allow(dead_code)]
fn lumie_fs_delete(path: *const u8) -> i32 {
    unsafe { fs::delete(path) }
}

#[allow(dead_code)]
fn fat_delete(path: *const u8) -> i32 {
    lumie_fs_delete(path)
}

unsafe fn gpu_api_fill_rect(x: u32, y: u32, w: u32, h: u32, color: u32) -> i32 {
    gfx::gfx_fill_rect(x, y, w, h, color);
    0
}
unsafe fn gpu_api_put_pixel(x: u32, y: u32, color: u32) -> i32 {
    gfx::gfx_put_pixel(x, y, color);
    0
}
unsafe fn gpu_api_get_pixel(x: u32, y: u32) -> u32 {
    gfx::gfx_get_pixel(x, y)
}
unsafe fn gpu_api_is_active() -> i32 {
    if gfx::gfx_ctx().gpu_active { 1 } else { 0 }
}
unsafe fn gpu_api_flip() {
    gfx::gfx_flip();
}
unsafe fn gpu_api_vsync() {
    gfx::gfx_vsync();
}
...
pub fn lumie_load_shell_module() -> i32 {
    ...
    let kapi = api::KernelApiV1 {
        version: crate::api::KAPI_VERSION,
        ...
        gpu_fill_rect: Some(gpu_api_fill_rect),
        gpu_put_pixel: Some(gpu_api_put_pixel),
        gpu_get_pixel: Some(gpu_api_get_pixel),
        gpu_is_active: Some(gpu_api_is_active),
        gpu_flip: Some(gpu_api_flip),
        gpu_vsync: Some(gpu_api_vsync),
    ...
    for &path in paths {
        let mut mod_: crate::globals::ModuleT = unsafe { core::mem::zeroed() };
        let rc = unsafe {
            system::module::module_load(
                path,
                &kapi as *const api::KernelApiV1 as *const c_void,
                &mut mod_ as *mut crate::globals::ModuleT as *mut crate::system::module::ModuleT,
            )
        };
        if rc == 0 {
            unsafe {
...
fn lumie_panic(msg: *const u8, file: *const u8, line: i32) {
    unsafe {
        console::term_clear(0x0000AA);
        console::term_set_fg(0xAAAAAA);
        console::term_set_bg(0x0000AA);

        let rows = console::term_get_height();

        console::term_set_pos(2, 2);
        console::term_set_fg(0xAAAAAA);
        console::term_write(b"LumieOS\0" as *const u8);
        console::term_set_fg(0x55FFFF);
        console::term_writeln(b" (Windows Edition)\0" as *const u8);

        console::term_set_fg(0xAAAAAA);
        console::term_set_pos(2, 4);
        console::term_write(b"A fatal error has occurred.\0" as *const u8);
        console::term_set_pos(2, 5);
        console::term_write(b"The operating system needs to restart.\0" as *const u8);

        console::term_set_pos(2, 7);
        console::term_write(b"Error: \0" as *const u8);
        console::term_writeln(msg);

        if !file.is_null() {
            console::term_set_pos(2, 9);
            console::term_write(b"At: \0" as *const u8);
            console::term_write(file);
            if line > 0 {
                let mut lbuf: [u8; 32] = [0u8; 32];
                lumie_std::format::lumie_itoa(line as i64, lbuf.as_mut_ptr(), 10);
                console::term_write(b":\0" as *const u8);
                console::term_write(lbuf.as_ptr());
            }
        }

        console::term_set_pos(2, rows - 3);
        console::term_write(b"Press any key to restart...\0" as *const u8);
...
pub fn lumie_reboot() {
    unsafe { console::term_writeln(b"Rebooting...\0" as *const u8); }
    lumie_stall(500000);
    if let Some(rt) = crate::globals::get_rt() {
        unsafe {
            if let Some(reset) = rt.reset_system {
...
#[allow(dead_code)]
fn lumie_shutdown() {
    unsafe { console::term_writeln(b"Shutting down...\0" as *const u8); }
    lumie_stall(500000);
    if let Some(rt) = crate::globals::get_rt() {
        unsafe {
            if let Some(reset) = rt.reset_system {
...
fn try_register_boot_entry(base_num: u16, attrs: u32) -> i32 {
    ...
    let status = unsafe {
        let hp = match bs.handle_protocol {
            Some(h) => h,
            None => return -1,
        };
        hp(
...
pub fn exit_boot_services() {
    ...
    for _ in 0..3 {
        ...
        if status == EFI_SUCCESS {
            unsafe { crate::globals::set_bs(None); }
            return;
        }
...
pub fn lumie_ps2_available() -> i32 {
...
pub fn lumie_cache_kernel_image(base: *const c_void, size: u32) {
    unsafe { crate::globals::set_kernel_image(base, size); }
}
...
impl LumieFmtPrinter {
    ...
    fn finish(&mut self) {
        self.buf[self.pos] = 0;
        unsafe { console::term_write(self.buf.as_ptr()); }
    }
}

pub unsafe fn lumie_printf(fmt: *const u8) {
