...
unsafe fn alloc_c(size: usize) -> *mut u8 {
    ...
    if let Some(alloc) = disk.alloc_cb {
        return alloc(size);
    }
    ptr::null_mut()
}
...
unsafe fn fat_read_fat_entry(cluster: u32) -> u32 {
    ...
    let sector = alloc_c(bpb.bytes_per_sector as usize);
    if sector.is_null() {
        return 0xFFFFFFFF;
    }
...
unsafe fn fat_find_cluster(path: *const u8, out_ent: *mut FatDirEnt) -> i32 {
    ...
    let sector = alloc_c(cluster_size as usize);
    if sector.is_null() {
        return 0;
    }
    let mut p = path;
...
pub unsafe fn read_file(path: *const u8, buffer: *mut u8, max_size: u32) -> i32 {
    ...
    let bpb: FatBpb = ptr::read_unaligned(&(*fat()).bpb as *const FatBpb);
    let cluster_size = (bpb.sectors_per_cluster as u32) * bpb.bytes_per_sector as u32;
    let temp = alloc_c(cluster_size as usize);
    if temp.is_null() {
        return -1;
    }
...
pub unsafe fn set_alloc(alloc_cb: Option<AllocFn>, free_cb: Option<FreeFn>) -> i32 {
    let drv = &mut *FAT_DRIVER.as_mut_ptr();
    drv.disk_io.alloc_cb = alloc_cb;
    drv.disk_io.free_cb = free_cb;
    0
}
